...
static void	load_player_assets_dir(t_gamestate *st, t_dir dir)
{
	...
	mlximg_from_xpm(st->mlx, "textures/pacMan.xpm", &st->assets.player[dir].base);
	img_clone(imgview(&st->assets.player[dir].base.img), &tmp);
	if (dir == LEFT)
		imgview_flip_h(imgview(&st->assets.player[dir].base.img));
	else if (dir == UP)
		imgview_rotate90(imgview(&tmp), imgview(&st->assets.player[dir].base.img));
	else if (dir == DOWN)
		imgview_rotate270(imgview(&tmp), imgview(&st->assets.player[dir].base.img));
	img_destroy(&tmp);
	mlximg_clone(st->mlx, imgview(&st->assets.player[dir].base.img), &st->assets.player_on_exit[dir].base);
	blend_asset_with_bg(st, &st->assets.exit.base, &st->assets.player_on_exit[dir].base);
	blend_asset_with_bg(st, &st->assets.floor.base, &st->assets.player[dir].base);
}
...
static void	update_scaled_player_asset(t_gamestate *st, t_dir dir)
{
	...
	mlximg_clone_scaled(st->mlx, imgview(&st->assets.player_on_exit[dir].base.img),
		st->opts.scale, &st->assets.player_on_exit[dir].scaled);
}
